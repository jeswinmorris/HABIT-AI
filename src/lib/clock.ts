...
/* ----------------------------- system authority ----------------------------- */
/**
 * The operating system clock is the only authority in this app: date, weekday, wall time,
 * locale formatting and timeZone all come from here, and the title bar shows it live so you
 * can see what the engine is working from. Nothing in src/** may call
 * `new Date().toISOString()` for a calendar day any more.
 */
...
export function systemClock(at: Date = new Date()): SystemClock {
  let timeZone = 'local'
  try { timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone || 'local' } catch {}
  return {
    dateKey: dayKey(at),
    weekday: weekdayOf(at),
    weekdayFull: at.toLocaleDateString([], { weekday: 'long' }),
    dateLabel: dateLabel(at),
    time24: clockWithSeconds(at),
    time12: at.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }),
    day: at.getDate(),
    month: at.getMonth() + 1,
    year: at.getFullYear(),
    minutes: minutesNow(at),
    timeZone,
    locale: typeof navigator !== 'undefined' ? navigator.language : 'en',
    utcOffsetMin: -at.getTimezoneOffset()
  }
}

/** The halfway point between two local HH:MM values (used for "more than half before the day is half over"). */
export function midpointOf(startHHMM: string, endHHMM: string): number {
  const a = toMinutes(startHHMM), b = toMinutes(endHHMM)
  if (Number.isNaN(a) || Number.isNaN(b)) return 13 * 60
  const span = b > a ? b - a : b + 1440 - a
  return (a + Math.floor(span / 2)) % 1440
}
export function midpointLabel(startHHMM: string, endHHMM: string): string { return toHHMM(midpointOf(startHHMM, endHHMM)) }

/** 0..1 through the waking day (start → bedtime). */
export function dayProgress(startHHMM: string, endHHMM: string, at: Date = new Date()): number {
  const a = toMinutes(startHHMM), b = toMinutes(endHHMM)
  if (Number.isNaN(a) || Number.isNaN(b) || b === a) return 0
  const span = b > a ? b - a : b + 1440 - a
  const done = (minutesNow(at) - a + 1440) % 1440
  return Math.max(0, Math.min(1.5, done / span))
}
/** How many minutes are left before the day's cut-off (bedtime), wrapping over midnight. */
export function minutesLeft(endHHMM: string, at: Date = new Date()): number {
  const b = toMinutes(endHHMM)
  if (Number.isNaN(b)) return 0
  return (b - minutesNow(at) + 1440) % 1440
}
/** "1h 24m" style span. */
...
/** Local timestamp used on records the engine writes (never UTC). */
export function localStamp(at: Date = new Date()): string {
  return `${dayKey(at)} ${clockNow(at)}`
}
