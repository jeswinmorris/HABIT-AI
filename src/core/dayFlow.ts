...
export type DayRecord = {
  ...
  alarmClosedAt: string | null
  endedAt: string | null
  snoozeUntil: number | null
  /** real close reason so the report can say 'closed by you' vs 'bedtime' */
...
export type DaySession = {
  ...
  planFired: number
  waterMl: number
  /** how many tasks applied that day — 0 until you record it */
  tasksTotal?: number
}
...
export function dayRecord(): DayRecord {
  const st = read() || {}
  let raw: DayRecord
  try { raw = { ...emptyDay(), ...(st.day && typeof st.day === 'object' ? st.day : {}) } } catch { raw = emptyDay() }
  if (!raw.day || typeof raw.day !== 'string') raw.day = todayKey()
...
export function startDay(): DayRecord {
  const cur = dayRecord()
  const rec = setDay({ phase: 'active', startedAt: cur.startedAt || new Date().toISOString(), snoozeUntil: null, endedAt: null, endReason: null })
  window.dispatchEvent(new CustomEvent('habit:day-started'))
  return rec
}
export function endDay(reason = 'manual', extra: Partial<DaySession> = {}): DayRecord {
  const cur = dayRecord()
  const rec = setDay({ phase: 'ended', endedAt: new Date().toISOString(), endReason: reason })
  recordSession({ ...rec, endReason: reason }, extra)
  return rec
}

/* ---------------- day sessions: the only thing the reports are allowed to read ----------- */
const SESSIONS = 'habitOS_daySessions'

export function readSessions(): DaySession[] {
  try {
    const raw = JSON.parse(readStorage(SESSIONS, '[]'))
    const all = Array.isArray(raw) ? raw : []
    // the day currently in progress shows up too, so graphs are live rather than nightly
    const cur = dayRecord()
    if (cur.startedAt && !all.some((d: DaySession) => d.day === cur.day)) {
      all.push({ day: cur.day, startedAt: cur.startedAt, endedAt: null, endReason: null, minutes: 0, tasksDone: 0, tasksTotal: 0, planFired: 0, waterMl: 0 })
    } else if (cur.startedAt) {
      all.splice(all.findIndex((d: DaySession) => d.day === cur.day), 1, {
        day: cur.day, startedAt: cur.startedAt, endedAt: cur.endedAt, endReason: cur.endReason || null,
        minutes: cur.endedAt ? Math.max(0, Math.round((Date.parse(cur.endedAt) - Date.parse(cur.startedAt)) / 60000))
          : Math.max(0, Math.round((Date.now() - Date.parse(cur.startedAt)) / 60000)),
        tasksDone: 0, planFired: 0, waterMl: 0
      })
    }
    return all
  } catch { return [] }
...
export function recordSession(rec: DayRecord, extra: Partial<DaySession> = {}): DaySession | null {
  if (!rec.startedAt) return null
  const minutes = rec.endedAt ? Math.max(0, Math.round((Date.parse(rec.endedAt) - Date.parse(rec.startedAt)) / 60000)) : 0
  const session: DaySession = {
    day: rec.day, startedAt: rec.startedAt, endedAt: rec.endedAt, endReason: rec.endReason || 'manual',
    minutes, tasksDone: extra.tasksDone || 0, planFired: extra.planFired || 0, waterMl: extra.waterMl || 0
  }
  try {
    const list = readStorage(SESSIONS, '[]')
    const all = (JSON.parse(list) || []).filter((d: DaySession) => d.day !== session.day)
    all.push(session)
    writeStorage(SESSIONS, JSON.stringify(all.slice(-180)))
  } catch {}
  return session
}
