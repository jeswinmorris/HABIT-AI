...
class PlaylistEngine {
  ...
  private volume = 0.8
  private muted = false
  /** brief fade-in / "mild" factor applied on top of the volume setting */
  private duck = 1
  ...
  playing = false
  /** the user wants music on (survives the bus pausing us for a reminder) */
  private intended = false

  setDuck(factor: number) {
    this.duck = Math.max(0.1, Math.min(1, factor))
    if (this.el) this.el.volume = this.muted ? 0 : this.volume * this.duck
  }
  configure(cfg: { tracks?: PlaylistTrack[]; mode?: PlayMode; volume?: number; muted?: boolean }) {
    ...
    if (typeof cfg.volume === 'number') { this.volume = Math.max(0, Math.min(1, cfg.volume)); if (this.el) this.el.volume = this.muted ? 0 : this.volume * this.duck }
    if (typeof cfg.muted === 'boolean') { this.muted = cfg.muted; if (this.el) this.el.volume = this.muted ? 0 : this.volume * this.duck }
    this.emit()
  }
  ...
  private audio(): HTMLAudioElement {
    if (!this.el) {
      const el = new Audio()
      el.preload = 'auto'
      el.volume = this.muted ? 0 : this.volume * this.duck
  ...
  private claim(): boolean {
    if (!audioBus.claim('music')) return false
    return true
  }
  ...
  async loadAndPlay(index = this.index): Promise<boolean> {
    ...
    if (!t) return false
    if (!this.claim()) { this.playing = false; this.emit(); return false }
    this.index = index
  ...
  /** the bus handing the speakers back after a reminder/affirmation finished */
  resumeFromBus() { if (this.intended) void this.resumeOrPlay() }
  pause() { this.intended = false; try { this.el?.pause() } catch {} this.playing = false; audioBus.release('music'); this.emit() }
  stop() {
    this.intended = false
    try { this.el?.pause(); if (this.el) this.el.currentTime = 0 } catch {}
    this.playing = false
    audioBus.release('music')
    this.emit()
  }
...
audioBus.register({
  channel: 'music',
  suspend: () => { try { (playlist as any).el?.pause() } catch {} (playlist as any).playing = false },
  maybeResume: () => playlist.resumeFromBus(),
  isWanted: () => (playlist as any).intended === true
})
