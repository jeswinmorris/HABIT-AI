...
type RitualTask = { id: string, title: string, instructions: string, imageUrl?: string, videoUrl?: string, mediaType?: 'image' | 'video', done: boolean, date: string }
...
export default function RitualsView() {
  ...
  const [tasks, setTasks] = useState<Record<string, RitualTask[]>>(() => { try { return JSON.parse(localStorage.getItem('habitOS_ritualTasks') || '{}') } catch { return {} } })
  const [prayerSlides, setPrayerSlides] = useState<PrayerSlide[]>(() => {
    try {
      const raw = JSON.parse(localStorage.getItem('habitOS_prayerSlides') || 'null')
      if (raw && Array.isArray(raw)) return raw.map(migratePrayerSlide)
      return [{ id: 'p1', title: 'Morning Guidance', texts: ['I am guided.'], imageUrl: '' }]
    } catch { return [] }
  })
  ...
  const [vizSlides, setVizSlides] = useState<VizSlide[]>(() => {
    try { return JSON.parse(localStorage.getItem('habitOS_vizSlides') || 'null') || [{ id: 'v1', title: 'Future self 1 year', text: 'See yourself healthy.', imageUrl: '', mediaType: 'image' }] } catch { return [] }
  })
  ...
  const [affirmations, setAffirmations] = useState<AffItem[]>(() => { try { return JSON.parse(localStorage.getItem('habitOS_affirmations') || 'null') || [{ id: 'a1', text: 'I am becoming my best self' }] } catch { return [] } })
  const [affAudios, setAffAudios] = useState<AudioRef[]>(() => { try { return JSON.parse(localStorage.getItem('habitOS_affAudios') || '[]') } catch { return [] } })
  ...
  useEffect(() => {
    ...
    const onPlay = () => setVizPlaying?.(true)
    const onPause = () => setVizPlaying?.(false)
  ...
  useEffect(() => { localStorage.setItem('habitOS_ritualTasks', JSON.stringify(tasks)) }, [tasks])
  ...
  useEffect(() => { localStorage.setItem('habitOS_vizSlides', JSON.stringify(vizSlides)) }, [vizSlides])
  ...
  const nextAffAudio = () => { const n = (affAudioIdx + 1) % Math.max(1, affAudios.length); setAffAudioIdx(n); setTimeout(() => affAudioRef.current?.play(), 120) }
  ...
  useEffect(() => {
    ...
    const onAff = (e: Event) => {
      const d = (e as CustomEvent).detail
      if (d === 'play') { setIsAffPlaying((p: boolean) => !p); affAudioRef.current?.play().catch(() => {}) }
      ...
      {ritualTab === 'Affirmations' && (
        <div className="rounded-2xl bg-[#141418] border border-white/10 p-4 md:p-5 space-y-4">
          <div className="flex items-center gap-2 flex-wrap"><Quote className="w-4 h-4 text-amber-300" /><span className="text-sm font-medium text-white">Affirmations</span>
            <button onClick={() => tickRitual('affirmations', !affirmDone)} className={`ml-auto text-xs px-3 py-1.5 rounded-full border flex items-center gap-1 ${affirmDone ? 'bg-emerald-500/15 border-emerald-500/25 text-emerald-200' : 'bg-white/5 border-white/10 text-white/45'}`}>
          ...
          <div ref={scrollRef} className="max-h-64 overflow-y-auto rounded-xl bg-[#0e0e12] border border-white/5 p-3 space-y-2">
            {affirmations.map(a => <div key={a.id} className="group flex items-center gap-2 p-3 rounded-xl bg-[#141418] border border-white/5"><span className="flex-1 text-sm text-white/70">✨ {a.text}</span><button onClick={() => setAffirmations(prev => prev.filter(x => x.id !== a.id))} className="opacity-0 group-hover:opacity-100 w-6 h-6 rounded-lg bg-white/5 flex items-center justify-center"><Trash2 className="w-3 h-3 text-white/30" /></button></div>)}
          </div>
          <AudioPool
            ...
            onPick={(i) => { setAffAudioIdx(i); setTimeout(() => affAudioRef.current?.play(), 120); setIsAffPlaying(true) }}
            onNext={nextAffAudio}
            onToggle={() => { if (isAffPlaying) { affAudioRef.current?.pause() } else { affAudioRef.current?.play() } setIsAffPlaying(p => !p) }}
...
function AudioPool({ title, audios, idx, playing, onAdd, onPick, onNext, onToggle, onRemove, audioRef, onEnded }: {
  ...
  return (
    <div className="rounded-xl bg-[#0e0e12] border border-white/10 p-4 space-y-3">
      ...
      <audio ref={audioRef} src={src || undefined} controls className="w-full h-8" onEnded={onEnded} />
      {audios.length > 0 && (
    ...
    return (
      <div className="rounded-2xl bg-[#141418] border border-white/10 p-4 md:p-5 space-y-5">
        ...
        <AudioPool
          ...
          onToggle={() => { const a = prayerAudioRef.current; if (!a) return; if (a.paused) { a.play(); setPlaying(true) } else { a.pause(); setPlaying(false) } }}
    ...
    return (
      <div className="rounded-2xl bg-[#141418] border border-white/10 p-4 md:p-5 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div key={t.id} className="rounded-xl bg-[#0e0e12] border border-white/5 p-3">
              {editTaskId === t.id ? (
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-medium text-white truncate">{i + 1}. {t.title}</div>
                    ...
                    <div className="flex gap-1 mt-2 flex-wrap">
                      ...
                      <button onClick={() => { setEditTaskId(t.id); setEditTaskTitle(t.title); setEditTaskInstr(t.instructions) }} className="px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-200 text-xs">Edit</button>
          ...
          {(tasks[ritualTab] || []).length === 0 && <div className="text-xs text-white/25 col-span-2 py-4">Nothing here yet — add your {ritualTab.toLowerCase()} routine below.</div>}
        </div>
        <div className="border-t border-white/5 pt-4 space-y-3">
          ...
          <button onClick={() => {
            ...
            setTasks((prev: any) => ({ ...prev, [ritualTab]: [...(prev[ritualTab] || []), { id: Date.now().toString(), title: newTaskTitle.trim(), instructions: newTaskInstr.trim(), imageUrl: newTaskMedia?.type === 'image' ? newTaskMedia.url : '', videoUrl: newTaskMedia?.type === 'video' ? newTaskMedia.url : '', mediaType: newTaskMedia?.type || 'image', done: false, date: today }] }))
            setNewTaskTitle(''); setNewTaskInstr(''); setNewTaskMedia(null)
