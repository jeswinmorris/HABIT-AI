...
import { Settings, Mic, User, Volume2, Download, Upload, Waves, Sliders, Activity, Zap, Check, Music, Shuffle, Repeat, Repeat1, Play, Pause, SkipForward, SkipBack, Sparkles, ListMusic, Trash2, Brain, X } from 'lucide-react'
import { personality } from '../../speaking/personality'
import { exportDB, importDB, dbInfo } from '../../lib/db'
import * as voiceEngine from '../../core/voiceEngine'
import { playlist } from '../../core/playlist'
import { putMedia, removeMedia, mediaId } from '../../lib/mediaStore'
import { WEEKDAYS } from '../../lib/clock'
...
export default function SettingsView(props: any) {
  ...
  const {
    ...
    affirmPlayout, secondaryNudges, activeDays, affirmGapMin, setGlobalMuted, globalMuted,
  ...
  useEffect(() => voiceEngine.onStatus((s: any) => setStt(s)), [])
  useEffect(() => {
    ...
    <div className="p-3 md:p-6 space-y-4 bg-[#0f0f12] min-h-full">
      ...
      <div className="rounded-2xl bg-[#141418] border border-white/10 p-4 md:p-5">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => exportDB().then(toast)} className="px-4 py-2 rounded-xl bg-white text-black text-xs font-semibold flex items-center gap-2"><Download className="w-3 h-3" /> Export JSON</button>
          <label className="px-4 py-2 rounded-xl bg-white/10 border border-white/10 text-white/60 text-xs flex items-center gap-2 cursor-pointer"><Upload className="w-3 h-3" /> Import<input type="file" hidden accept=".json" onChange={(e) => { const f = e.target.files?.[0]; if (!f) return; const r = new FileReader(); r.onload = async () => { try { const ok = await importDB(JSON.parse(String(r.result))); toast(ok ? 'Imported. Reloading…' : 'Not a habit backup'); if (ok) setTimeout(() => location.reload(), 700) } catch { toast('Invalid JSON') } }; r.readAsText(f) }} /></label>
          <button onClick={() => { if (confirm('Wipe steaks, tasks, logs and skips? Settings + playlist stay.')) resetHabitData() }} className="px-4 py-2 rounded-xl bg-red-500/10 border border-red-500/20 text-red-200 text-xs flex items-center gap-2"><Trash2 className="w-3 h-3" /> Clean data</button>
          ...
          <button className="px-4 py-2 rounded-xl bg-[#0e0e12] border border-white/10 text-white/60 text-xs flex items-center gap-2"><Activity className="w-3 h-3" /> {dayPaused ? 'PAUSED' : 'Engine live'}</button>
        </div>
